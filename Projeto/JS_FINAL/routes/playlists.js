...
router.get('/:id/comentarios', autenticacao.userTipo(["admin","consumidor","produtor"]), function(req, res, next) {
    ...
    .then(comentarios => {
        ...
        .then(user => {
            res.status(200).jsonp({comentarios: comentarios.comentarios, user: user})
        })
    })
});
...
router.get('/:id', autenticacao.userTipo(["admin","consumidor","produtor"]), function(req, res, next) {
    ...
    .then(dados => {
        ...
        .then(async nome => {
            ...
            res.render('playlist',{title: "Playlist", playlist: dados, user: nome, musicas: musicas})
        })
    })
    .catch(err => {
        res.render('error',{error: err})
...
router.post('/comentario/:id', autenticacao.userTipo(["admin","consumidor","produtor"]), function(req, res, next) {
    User.getById(req.userId)
    .then(user => {
        Playlist.addComment(req.params.id, req.body.comentario, user.nome)
        .then(_ => {
        ...
        Playlist.get(req.params.id)
        .then(playlist => {
            if (playlist.autor!=user.nome) {
                var notificacao = {
                    autor: user.nome,
                    artista: playlist.autor,
                    fonte: "playlist",
